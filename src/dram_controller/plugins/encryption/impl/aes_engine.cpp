#include "../aes_engine.h"
#include <cstring>

namespace Ramulator {
    const uint8_t AESEngine::S_BOX[256] = {
        
    };

    const uint8_t AESEngine::INV_S_BOX[256] = {
        
    };

    const uint8_t AESEngine::RCON[11]={
        0x00, 0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1B, 0x36
    };

    AESEngine::AESEngine() : m_initialized(false) {
        m_params.Nb = 4;
    }

    bool AESEngine::initialize(const uint8_t* key, int key_size) {
        switch (key_size) {
            case AES_KEY_SIZE_128:
                m_params.Nk = 4;
                m_params.Nr = 10;
                break;
            case AES_KEY_SIZE_192:
                m_params.Nk = 6;
                m_params.Nr = 12;
                break;
            case AES_KEY_SIZE_256:
                m_params.Nk = 8;
                m_params.Nr = 14;
                break;
            default:
                return false;
        }

        try {
            keyExpansion(key, key_size);
            m_initialized = true;
            return true;
        }catch (const std::exception& e) {
            m_initialized = false;
            return false;
        }
    }

    bool AESEngine::initialize(const std::vector<uint8_t>& key) {
        return initialize(key.data(), key.size());
    }

    bool AESEngine::encrypt(const uint8_t* input, uint8_t* output) {
        if (!m_initialized) {
            return false;
        }

        uint8_t state[4][4];
        bytesToState(input, state);

        addRoundKey(state, &m_expanded_keys[0]);

        for (int round = 1; round < m_params.Nr; round++) {
            subBytes(state);
            shiftRow(state);
            mixColumn(state);
            addRoundKey(state, &m_expanded_keys[round * m_params.Nb]);
        }

        subBytes(state);
        shiftRow(state);
        addRoundKey(state, &m_expanded_keys[m_params.Nr * m_params.Nb]);

        stateToBytes(state, output);
        return true;
    }

    bool AESEngine::decrypt(const uint8_t* input, uint8_t* output) {
        if (!m_initialized) {
            return false;
        }

        uint8_t state[4][4];
        bytesToState(input, state);

        addRoundKey(state, &m_expanded_keys[m_params.Nr * m_params.Nb]);

        for (int round = m_params.Nr - 1; round >= 1; round--) {
            invShiftRow(state);
            invSubBytes(state);
            addRoundKey(state, &m_expanded_keys[round * m_params.Nb]);
            invMixColumn(state);
        }

        invShiftRow(state);
        invSubBytes(state);
        addRoundKey(state, &m_expanded_keys[0]);

        stateToBytes(state, output);
        return true;
    }

    void AESEngine::subBytes(uint8_t state[4][4]) {
        for (int i = 0; i < 4; i++) {
            for (int j = 0; j < 4; j++) {
                state[i][j] = S_BOX[state[i][j]];
            }
        }
    }

    void AESEngine::invSubBytes(uint8_t state[4][4]) {
        for (int i = 0; i < 4; i++) {
            for (int j = 0; j < 4; j++) {
                state[i][j] = INV_S_BOX[state[i][j]];
            }
        }
    }

    void AESEngine::shiftRow(uint8_t state[4][4]) {
        uint8_t temp;

        temp = state[1][0];
        state[1][0] = state[1][1];
        state[1][1] = state[1][2];
        state[1][2] = state[1][3];
        state[1][3] = temp;

        uint8_t temp1 = state[2][0];
        uint8_t temp2 = state[2][1];
        state[2][0] = state[2][2];
        state[2][1] = state[2][3];
        state[2][2] = temp1;
        state[2][3] = temp2;

        temp = state[3][0];
        state[3][0] = state[3][3];
        state[3][3] = state[3][2];
        state[3][2] = state[3][1];
        state[3][1] = temp;
    }

    void AESEngine::invShiftRow(uint8_t state[4][4]) {
        uint8_t temp;

        temp = state[1][3];
        state[1][3] = state[1][2];
        state[1][2] = state[1][1];
        state[1][1] = state[1][0];
        state[1][0] = temp;

        uint8_t temp1 = state[2][2];
        uint8_t temp2 = state[2][3];
        state[2][3] = state[2][1];
        state[2][2] = state[2][0];
        state[2][1] = temp2;
        state[2][0] = temp1;

        temp = state[3][0];
        state[3][0] = state[3][1];
        state[3][1] = state[3][2];
        state[3][2] = state[3][3];
        state[3][3] = temp;
    }

    void AESEngine::mixColumn(uint8_t state[4][4]) {
        for (int c = 0; c < 4; c++) {
            uint8_t temp_col[4];
            for (int i= 0; i<4; i++) {
                temp_col[i] = state[i][c];
            }

            state[0][c] = gf_mul(0x02, temp_col[0]) ^ gf_mul(0x03, temp_col[1]) ^ temp_col[2] ^ temp_col[3];
            state[1][c] = temp_col[0] ^ gf_mul(0x02, temp_col[1]) ^ gf_mul(0x03, temp_col[2]) ^ temp_col[3];
            state[2][c] = temp_col[0] ^ temp_col[1] ^ gf_mul(0x02, temp_col[2]) ^ gf_mul(0x03, temp_col[3]);
            state[3][c] = gf_mul(0x03, temp_col[0]) ^ temp_col[1] ^ temp_col[2] ^ gf_mul(0x02, temp_col[3]);
        }
    }

    void AESEngine::invMixColumn(uint8_t state[4][4]) {
        for (int c = 0; c < 4; c++) {
            uint8_t temp_col[4];
            for (int i = 0; i < 4; i++) {
                temp_col[i] = state[i][c];
            }

            state[0][c] = gf_mul(0x0E, temp_col[0]) ^ gf_mul(0x0B, temp_col[1]) ^ gf_mul(0x0D, temp_col[2]) ^ gf_mul(0x09, temp_col[3]);
            state[1][c] = gf_mul(0x09, temp_col[0]) ^ gf_mul(0x0E, temp_col[1]) ^ gf_mul(0x0B, temp_col[2]) ^ gf_mul(0x0D, temp_col[3]);
            state[2][c] = gf_mul(0x0D, temp_col[0]) ^ gf_mul(0x09, temp_col[1]) ^ gf_mul(0x0E, temp_col[2]) ^ gf_mul(0x0B, temp_col[3]);
            state[3][c] = gf_mul(0x0B, temp_col[0]) ^ gf_mul(0x0D, temp_col[1]) ^ gf_mul(0x09, temp_col[2]) ^ gf_mul(0x0E, temp_col[3]);
        }
    }

    void AESEngine::addRoundKey(uint8_t state[4][4], const uint32_t* round_key) {
        for (int i = 0; i < 4; i++) {
            for (int j = 0; j < 4; j++) {
                state[i][j] ^= (round_key[j] >> (24-8*i)) & 0xFF;
            }
        }
    }

    uint8_t AESEngine::gf_mul(uint8_t a, uint8_t b) {
        uint8_t result = 0;
        uint8_t high_bit;
        for (int i = 0; i < 8; i++) {
            if (b&1) {
                result ^= a;
            }
            high_bit = a&0x80;
            a <<= 1;
            if (high_bit) {
                a ^= 0x1B; //the irreducible polynimial
            }
            b >>= 1;
        }
        return result;
    }

    void AESEngine::keyExpansion(const uint8_t* key, int key_size) {
        int total_words = m_params.Nb * (m_params.Nr + 1);
        m_expanded_keys.resize(total_words);

        //copy original key
        for (int i = 0; i < m_params.Nk; i++) {
            m_expanded_keys[i] = bytesToWord(key[4*i], key[4*i+1], key[4*i+2], key[4*i+3]);
        }

        //generate rest of keys
        for (int i = m_params.Nk; i<total_words; i++) {
            uint32_t temp = m_expanded_keys[i - 1];
            if (i % m_params.Nk ==0) {
                temp = subWord(rotateWord(temp)) ^ (RCON[i/m_params.Nk] << 24);
            }else if (m_params.Nk > 6 && i%m_params.Nk == 4) {
                temp = subWord(temp);
            }

            m_expanded_keys[i] = m_expanded_keys[i - m_params.Nk] ^ temp;
        }
    }

    uint32_t AESEngine::subWord(uint32_t word) {
        return (S_BOX[(word >> 24) & 0xFF] << 24) |
               (S_BOX[(word >> 16) & 0xFF] << 16) |
               (S_BOX[(word >> 8) & 0xFF] << 8) |
               S_BOX[word & 0xFF];
    }

    uint32_t AESEngine::rotateWord(uint32_t word) {
        return (word << 8) | (word >> 24);
    }

    void AESEngine::bytesToState(const uint8_t* input, uint8_t state[4][4]) {
        for (int i = 0; i < 4; i++) {
            for (int j = 0; j < 4; j++) {
                state[i][j] = input[i + 4 * j];
            }
        }
    }

    void AESEngine::stateToBytes(const uint8_t state[4][4], uint8_t* output) {
        for (int i = 0; i < 4; i++) {
            for (int j = 0; j < 4; j++) {
                output[i + 4 * j] = state[i][j];
            }
        }
    }

    uint32_t AESEngine::bytesToWord(uint8_t b0, uint8_t b1, uint8_t b2, uint8_t b3) {
        return (b0 << 24) | (b1 << 16) | (b2 << 8) | b3;
    }

    bool AESEngine::encryptBlock(const std::vector<uint8_t>& input, std::vector<uint8_t>& output) {
        if (input.size() != AES_BLOCK_SIZE) return false;

        output.resize(AES_BLOCK_SIZE);
        return encrypt(input.data(), output.data());
    }

    bool AESEngine::decryptBlock(const std::vector<uint8_t>& input, std::vector<uint8_t>& output) {
        if (input.size() != AES_BLOCK_SIZE) return false;

        output.resize(AES_BLOCK_SIZE);
        return decrypt(input.data(), output.data());
    }

    int AESEngine::getKeySize() const {
        if (!m_initialized) return 0;
        return m_params.Nk * 4;
    }


}

